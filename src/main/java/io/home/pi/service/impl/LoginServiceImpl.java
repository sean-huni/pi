...
import static io.home.pi.constant.SecurityCons.USER_ROLE_PREFIX;
...
public class LoginServiceImpl implements UserDetailsService {
    ...
    private List<GrantedAuthority> getGrantedAuthorities(io.home.pi.domain.User user) {
        ...
        try {
            List<GroupMember> groupMembers = groupMemberRepo.findByUsername(user.getUsername());
    ...
    private List<GroupAuthority> decodeHashMap(List<GroupAuthority> authorities) {
        List<GroupAuthority> authoritiesArrayList = new ArrayList<>();
        ObjectMapper mapper = new ObjectMapper();

        try {
            List<GroupAuthority> groupAuthorities = mapper.convertValue(authorities,
                    new TypeReference<List<GroupAuthority>>() {
                    });
